...
function App() {
  ...
  return (
    ...
    <nav className=" flex items-center justify-center item-center mt-6" >
      <h1 className="text-lime-400">Todo App</h1>
      <form className="flex justify-center item-center" onSubmit={addTodo}>
        <input className="border-2 border-rose-500" type="text" placeholder="enter text" onChange={(e) => setText(e.target.value)} value={text} />
        <button className='border-red-800 text-lime-400 border-4 border-indigo-400/100' type="submit">addTodo</button>
      </form>
        </nav>
      <ul className="mt-5">
        {todo.map((item, index) => {
